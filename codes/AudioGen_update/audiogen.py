...
class AudioGen:
    ...
    def _generate_tokens(self, attributes: tp.List[ConditioningAttributes],
                         prompt_tokens: tp.Optional[torch.Tensor], progress: bool = False,
                          prompt_tokens_onlyimp: tp.Optional[torch.Tensor] = None,gen_mode: str = "basic", alpha: float = 1.0) -> torch.Tensor:
        ...
        if self.duration <= self.max_duration:
            with self.autocast:
                if gen_mode == "audioXLM":
                    gen_tokens = self.lm.generate_audioXLM(     
                        prompt_tokens, attributes, prompt_tokens_onlyimp=prompt_tokens_onlyimp,
    ...
    def _generate_tokens_AudioXLM(self, attributes: tp.List[ConditioningAttributes],
                         prompt_tokens: tp.Optional[torch.Tensor], progress: bool = False,
                          prompt_tokens_onlyimp: tp.Optional[torch.Tensor] = None, gen_mode: str = "audioXLM", alpha: float = 1.0) -> torch.Tensor:
        """Generate discrete audio tokens given audio prompt and/or conditions.
